  ...
  <div class="space-y-6">
    ...
    <div class="rounded-xl border border-yellow-400/20 bg-yellow-400/5 p-4">
      <div class="flex items-center gap-2">
        <span class="text-yellow-200 font-medium">This list is in beta and will be updated soon</span>
      </div>
    </div>
    ...
    <div v-if="filteredClients.length === 0" class="flex flex-col items-center justify-center py-16">
      <div class="text-slate-500 mb-2">
        <svg class="h-12 w-12 mx-auto" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.75 17L9 20l-1 1h8l-1-1-.75-3M3 13h18M5 17h14a2 2 0 002-2V5a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
        </svg>
      </div>
      <p class="text-slate-400 text-center">No clients found matching your filters</p>
    </div>
        ...
        <div class="p-6">
          ...
          <div class="mb-4">
            ...
            <div class="space-y-3">
              <!-- Media Types -->
              <div>
                <h5 class="text-xs font-medium text-slate-400 mb-1">Media Support</h5>
                <div class="grid grid-cols-2 gap-1 text-xs">
                  <div class="flex items-center gap-1" :class="getFeatureClass(client.features.audiobooks)">
                    <component :is="getFeatureIcon(client.features.audiobooks)" class="h-3 w-3" />
                    Audiobooks
                  </div>
                  <div class="flex items-center gap-1" :class="getFeatureClass(client.features.podcasts)">
                    <component :is="getFeatureIcon(client.features.podcasts)" class="h-3 w-3" />
                    Podcasts
                  </div>
                  <div class="flex items-center gap-1" :class="getFeatureClass(client.features.ebooks?.available)">
                    <component :is="getFeatureIcon(client.features.ebooks?.available)" class="h-3 w-3" />
                    E-books
                  </div>
                </div>
                
                <!-- E-book subcategories -->
                <div v-if="client.features.ebooks?.available" class="ml-4 mt-1 grid grid-cols-2 gap-1 text-xs">
                  <div class="flex items-center gap-1" :class="getFeatureClass(client.features.ebooks?.ePubSupport)">
                    <component :is="getFeatureIcon(client.features.ebooks?.ePubSupport)" class="h-3 w-3" />
                    ePub Support
                  </div>
                  <div class="flex items-center gap-1" :class="getFeatureClass(client.features.ebooks?.pdfSupport)">
                    <component :is="getFeatureIcon(client.features.ebooks?.pdfSupport)" class="h-3 w-3" />
                    PDF Support
                  </div>
                  <div class="flex items-center gap-1" :class="getFeatureClass(client.features.ebooks?.annotations)">
                    <component :is="getFeatureIcon(client.features.ebooks?.annotations)" class="h-3 w-3" />
                    Annotations
                  </div>
                </div>
              </div>
              ...
              <!-- Playback Features -->
              <div>
                <h5 class="text-xs font-medium text-slate-400 mb-1">Playback</h5>
                <div class="grid grid-cols-2 gap-1 text-xs">
                  <div class="flex items-center gap-1" :class="getFeatureClass(client.features.queue)">
                    <component :is="getFeatureIcon(client.features.queue)" class="h-3 w-3" />
                    Queue
                  </div>
                  <div class="flex items-center gap-1" :class="getFeatureClass(client.features.autoqueue)">
                    <component :is="getFeatureIcon(client.features.autoqueue)" class="h-3 w-3" />
                    Auto Queue
                  </div>
                  <div class="flex items-center gap-1" :class="getFeatureClass(client.features.sleepTimer)">
                    <component :is="getFeatureIcon(client.features.sleepTimer)" class="h-3 w-3" />
                    Sleep Timer
                  </div>
                  <div class="flex items-center gap-1" :class="getFeatureClass(client.features.chapters)">
                    <component :is="getFeatureIcon(client.features.chapters)" class="h-3 w-3" />
                    Chapters
                  </div>
                  <div class="flex items-center gap-1" :class="getFeatureClass(client.features.playHistory)">
                    <component :is="getFeatureIcon(client.features.playHistory)" class="h-3 w-3" />
                    Play History
                  </div>
                  <div class="flex items-center gap-1" :class="getFeatureClass(client.features.shakeToRewind)">
                    <component :is="getFeatureIcon(client.features.shakeToRewind)" class="h-3 w-3" />
                    Shake to Rewind
                  </div>
                </div>
              </div>
              ...
              <!-- Technical Features -->
              <div>
                <h5 class="text-xs font-medium text-slate-400 mb-1">Technical</h5>
                <div class="grid grid-cols-2 gap-1 text-xs">
                  <div class="flex items-center gap-1" :class="getFeatureClass(client.features.downloads)">
                    <component :is="getFeatureIcon(client.features.downloads)" class="h-3 w-3" />
                    Downloads
                  </div>
                  <div class="flex items-center gap-1" :class="getFeatureClass(client.features.customHeaders)">
                    <component :is="getFeatureIcon(client.features.customHeaders)" class="h-3 w-3" />
                    Custom Headers
                  </div>
                  <div class="flex items-center gap-1" :class="getFeatureClass(client.features.caching)">
                    <component :is="getFeatureIcon(client.features.caching)" class="h-3 w-3" />
                    Caching
                  </div>
                  <div class="flex items-center gap-1" :class="getFeatureClass(client.features.biggerScreens)">
                    <component :is="getFeatureIcon(client.features.biggerScreens)" class="h-3 w-3" />
                    Bigger Screens
                  </div>
                  <div class="flex items-center gap-1" :class="getFeatureClass(client.features.oidc)">
                    <component :is="getFeatureIcon(client.features.oidc)" class="h-3 w-3" />
                    OIDC
                  </div>
          ...
          <div v-if="client.features.tested.date" class="text-xs text-slate-500">
...
const osFilterOptions = computed<FilterOption[]>(() => {
  ...
  return Array.from(oses).sort().map(os => ({
    value: os,
    label: os
  }))
})
