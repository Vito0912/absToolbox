      ...
      >
        <div class="space-y-3">
          <div class="flex items-start justify-between gap-3">
            <h3 class="font-semibold text-slate-100 group-hover:text-indigo-300 transition line-clamp-2">
              {{ project.name }}
            </h3>
            <div v-if="project.githubStats" class="flex items-center gap-1 text-xs text-slate-500 whitespace-nowrap">
          ...
          <div class="text-xs text-slate-500">
            ...
            <span v-for="(author, idx) in project.authors" :key="author">
              <span class="text-slate-400">{{ author }}</span><span v-if="idx < project.authors.length - 1">, </span>
            </span>
          </div>
          ...
          <div v-if="project.githubStats" class="flex items-center gap-3 text-xs text-slate-500">
            ...
            <div v-if="project.githubStats.language" class="flex items-center gap-1">
...
const filteredProjects = computed(() => {
  ...
  if (filters.value.search) {
    ...
    filtered = filtered.filter(project =>
      ...
      project.authors.some(author => author.toLowerCase().includes(search)) ||
