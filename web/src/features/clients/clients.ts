import type { ClientInfo } from "./types";

export const projects: ClientInfo[] = [
  {
    name: "Audiobookshelf PWA",
    link: "https://www.audiobookshelf.org/",
    iconLink: "./logos/ABS.png",
    language: "JS, Vue",
    OSes: ["Android", "iOS", "Web"],
    openSource: true,
    cost: "Free",
    notes: [
      {
        color: "blue",
        text: "Custom headers are checked, because you can use Forward Auth",
      },
      {
        color: "blue",
        text: "To use the PWA on iOS, you have to add it to the home screen from Safari",
      },
    ],
    features: {
      tested: {
        wasTested: true,
        date: "2025-09-07",
        usedApiCorrectly: 5,
      },
      ebooks: {
        available: true,
        pdfSupport: true,
        ePubSupport: true,
        annotations: false,
      },
      audiobooks: true,
      podcasts: true,
      queue: true,
      autoqueue: true,
      sleepTimer: true,
      chapters: true,
      playHistory: false,
      shakeToRewind: false,
      downloads: false,
      customHeaders: true,
      caching: false,
      biggerScreens: true,
      oidc: {
        available: true,
        mobileRedirectURI: "audiobookshelf://oauth",
      },
      carSupport: false,
      widgets: false,
      bookmarks: true,
    },
  },
  {
    name: "Audiobookshelf",
    link: "https://www.audiobookshelf.org/",
    iconLink: "./logos/ABS.png",
    language: "Kotlin, Swift, Vue",
    OSes: ["Android", "iOS"],
    openSource: true,
    cost: "Free",
    notes: [
      {
        color: "blue",
        text: "The iOS app does not support CarPlay. Downloads do not resume in the background.",
      },
      {
        color: "yellow",
        text: "There are known sync issues when listening offline or with Android Auto.",
      },
      {
        color: "red",
        text: "The iOS beta is currently full. Join the ABS Discord to get notified when new spots open.",
      },
    ],
    features: {
      tested: {
        wasTested: true,
        date: "2025-09-04",
        usedApiCorrectly: 5,
      },
      ebooks: {
        available: true,
        pdfSupport: true,
        ePubSupport: true,
        annotations: false,
      },
      audiobooks: true,
      podcasts: true,
      queue: false,
      autoqueue: false,
      sleepTimer: true,
      chapters: true,
      playHistory: true,
      shakeToRewind: true,
      downloads: true,
      customHeaders: false,
      caching: false,
      biggerScreens: true,
      oidc: {
        available: true,
        mobileRedirectURI: "audiobookshelf://oauth",
      },
      carSupport: true,
      widgets: true,
      bookmarks: true,
    },
  },
  {
    name: "Buchable (now Yaabsa)",
    link: "https://github.com/Vito0912/yaabsa",
    iconLink: "./logos/yaabsa.png",
    language: "Dart",
    OSes: ["Android", "Windows", "Linux", "iOS", "macOS"],
    openSource: true,
    cost: "Free",
    notes: [
      {
        color: "yellow",
        text: "This app was developed by me. This should not be a site to promote my apps, but rather provide a list of all available clients.",
      },
      {
        color: "green",
        text: "Has advanced features like server management, subtitles, syncable annotations",
      },
      {
        color: "blue",
        text: "Designed to be used with desktops, tablets, and phones",
      },
    ],
    features: {
      tested: {
        wasTested: true,
        date: "2026-05-05",
        usedApiCorrectly: 5,
        comments: [
          "Take that with a grain of salt, because I am the developer",
        ],
      },
      ebooks: {
        available: true,
        pdfSupport: true,
        ePubSupport: true,
        annotations: true,
      },
      audiobooks: true,
      podcasts: true,
      queue: true,
      autoqueue: true,
      sleepTimer: true,
      chapters: true,
      playHistory: true,
      shakeToRewind: true,
      downloads: true,
      customHeaders: true,
      caching: true,
      biggerScreens: true,
      oidc: {
        available: false,
        mobileRedirectURI: "yaabsa://oauth",
      },
      carSupport: true,
      widgets: true,
      bookmarks: true,
    },
  },
  {
    name: "Shelfplayer",
    link: "https://github.com/rasmuslos/ShelfPlayer",
    iconLink: undefined,
    language: "Swift",
    OSes: ["iOS"],
    openSource: true,
    cost: "Paid",
    notes: [
      {
        color: "blue",
        text: "Any features are only from the GitHub page, as I have not tested this app myself",
      },
    ],
    features: {
      tested: {
        wasTested: false,
        date: undefined,
        usedApiCorrectly: undefined,
        comments: [],
      },
      ebooks: {
        available: true,
        ePubSupport: false,
        pdfSupport: true,
        annotations: false,
      },
      audiobooks: true,
      podcasts: undefined,
      queue: true,
      autoqueue: undefined,
      sleepTimer: false,
      chapters: true,
      playHistory: undefined,
      shakeToRewind: undefined,
      downloads: true,
      customHeaders: undefined,
      caching: undefined,
      biggerScreens: true,
      oidc: {
        available: false,
      },
      carSupport: true,
      widgets: true,
      bookmarks: undefined,
    },
  },
  {
    name: "Plappa",
    link: "https://github.com/LeoKlaus/plappa",
    iconLink: undefined,
    language: "Unknown",
    OSes: ["iOS"],
    openSource: false,
    cost: "Freemium",
    notes: [
      {
        color: "green",
        text: "Supports watchOS (from what I read on the GitHub page)",
      },
      {
        color: "yellow",
        text: "Always uses /download for streaming books which can lead to issues and blocks listening without download permission. See https://github.com/advplyr/audiobookshelf/issues/4196#issuecomment-2797497694",
      },
      {
        color: "blue",
        text: "The libraries are in another sort order",
      },
      {
        color: "red",
        text: "There have been multiple reports of users that they have to log in every 7 days. This is likely a wrong implementation of the new auth system. Time of writing: 17.10.2025",
      },
    ],
    features: {
      tested: {
        wasTested: true,
        date: "09-06-2025",
        usedApiCorrectly: 2,
        comments: [
          "See red comment",
          "Can cause race conditions due to syncing twice at the same time (Will be fixed after ABS is restarted in recent ABS versions)",
          "Calls the user object every ~10 seconds that can be up to 1MB in size, additional to streaming a book",
          "Does not use the /play endpoint, but uses sessions",
        ],
      },
      ebooks: {
        available: false,
        ePubSupport: false,
        pdfSupport: false,
        annotations: false,
      },
      audiobooks: true,
      podcasts: false,
      queue: true,
      autoqueue: true,
      sleepTimer: true,
      chapters: true,
      playHistory: false,
      shakeToRewind: undefined,
      downloads: true,
      customHeaders: true,
      caching: undefined,
      biggerScreens: true,
      oidc: {
        available: true,
        mobileRedirectURI: "plappa://oauth",
      },
      carSupport: undefined,
      widgets: true,
      bookmarks: true,
    },
  },
  {
    name: "Lissen",
    link: "https://github.com/GrakovNe/lissen-android",
    iconLink: "./logos/lissen.png",
    language: "Kotlin",
    OSes: ["Android"],
    openSource: true,
    cost: "Free",
    notes: [
      {
        color: "green",
        text: "Supports Android Auto in a beta branch (but currently not in the Play Store version)",
      },
      {
        color: "blue",
        text: 'I would give this a score of 2.5, because only the download endpoint is problematic. Creating a session every time is not ideal, and syncing when not actively listening is also not good, but not "critical" and due to the working design (and awesome download option), I think a score of 2 would be unfair.',
      },
    ],
    features: {
      tested: {
        wasTested: true,
        date: "09-06-2025",
        usedApiCorrectly: 3,
        comments: [
          "Uses old playback API",
          "Uses a non-download endpoint for downloads, which allows users to download even without download permission and incorrect logging",
          "Does sync even when paused, which can overwrite progress made on other clients",
          "Creates a new session for every chapter",
        ],
      },
      ebooks: {
        available: false,
        ePubSupport: false,
        pdfSupport: false,
        annotations: false,
      },
      audiobooks: true,
      podcasts: true,
      queue: false,
      autoqueue: false,
      sleepTimer: true,
      chapters: true,
      playHistory: false,
      shakeToRewind: false,
      downloads: true,
      customHeaders: true,
      caching: undefined,
      biggerScreens: true,
      oidc: {
        available: true,
        mobileRedirectURI: "lissen://oauth",
      },
      carSupport: true,
      widgets: true,
      bookmarks: false,
    },
  },
  {
    name: "LitLyric",
    link: "https://github.com/shane9b3/LitLyric---Beta",
    iconLink: undefined,
    language: "Unknown",
    OSes: ["Android"],
    openSource: false,
    cost: "Free",
    notes: [
      {
        color: "red",
        text: "Needs full file system access for downloads to work",
      },
    ],
    features: {
      tested: {
        wasTested: true,
        date: "2025-12-07",
        usedApiCorrectly: 3,
        comments: [
          "Uses a non-download endpoint for downloads, which allows users to download with incorrect logging",
          "Uses an old playback API",
          "Does use progress and sessions, which can lead to problems with marking items as finished",
        ],
      },
      ebooks: {
        available: true,
        ePubSupport: true,
        pdfSupport: undefined,
        annotations: false,
      },
      audiobooks: true,
      podcasts: true,
      queue: false,
      autoqueue: false,
      sleepTimer: true,
      chapters: true,
      playHistory: false,
      shakeToRewind: false,
      downloads: true,
      customHeaders: true,
      caching: undefined,
      biggerScreens: false,
      oidc: {
        available: true,
        mobileRedirectURI: undefined,
      },
      carSupport: true,
      widgets: false,
      bookmarks: false,
    },
  },
  {
    name: "Tonspur",
    link: "https://tonspur.app/",
    iconLink: undefined,
    language: "Unknown",
    OSes: ["iOS"],
    openSource: false,
    cost: "Freemium",
    notes: [
      {
        color: "yellow",
        text: "The download feature can override progress made on other clients. Start a non-downloaded book before closing the app.",
      },
    ],
    features: {
      tested: {
        wasTested: true,
        date: "2025-09-28",
        usedApiCorrectly: 5,
        comments: [
          "Does not close sessions on it's own. Due to the stable implementation a new playback, closes the old session. 4.5/5 ",
        ],
      },
      ebooks: {
        available: false,
      },
      audiobooks: true,
      podcasts: false,
      queue: true,
      autoqueue: false,
      sleepTimer: true,
      chapters: true,
      playHistory: false,
      shakeToRewind: false,
      downloads: true,
      customHeaders: true,
      caching: true,
      biggerScreens: false,
      oidc: {
        available: true,
        mobileRedirectURI: "tonspur://oauth",
      },
      carSupport: true,
      widgets: false,
      bookmarks: false,
    },
  },
  {
    name: "Kitzi ABS Player",
    link: "https://github.com/bennybar/kitzi_abs_player/tree/v0.0.5",
    iconLink: undefined,
    language: "Dart",
    OSes: ["Android"],
    openSource: true,
    cost: "Free",
    notes: [
      {
        color: "green",
        text: "Dev is very responsive. After reporting major issues with the use of sessions, they quickly fixed most of them.",
      },
      {
        color: "red",
        text: "Signing out does not work/ leaves the app in a broken state",
      },
    ],
    heavyAiUsage: true,
    features: {
      tested: {
        wasTested: true,
        date: "2025-11-30",
        usedApiCorrectly: 0,
        comments: [
          "Testing 1.5.32 the app uses non-existing queries and wrong page offsets, making it impossible for me to test the app at all.",
          "Does load big chunks of books in advance.",
          "Opens sessions for everything you click on, even if you don't play it (but closes them)",
        ],
      },
      ebooks: {
        available: false,
        ePubSupport: false,
        pdfSupport: false,
        annotations: false,
      },
      audiobooks: true,
      podcasts: false,
      queue: false,
      autoqueue: false,
      sleepTimer: false,
      chapters: true,
      playHistory: false,
      shakeToRewind: false,
      downloads: true,
      customHeaders: false,
      caching: false,
      biggerScreens: undefined,
      oidc: {
        available: false,
      },
      carSupport: true,
      widgets: false,
      bookmarks: false,
    },
  },
  {
    name: "Soundleaf",
    link: "https://github.com/SoundLeaf/SoundLeafApp",
    iconLink: undefined,
    language: "Unknown",
    OSes: ["iOS"],
    openSource: false,
    cost: "Freemium",
    notes: [
      {
        color: "blue",
        text: "Switching libraries seems to be a multi step process, so users with multiple libraries might find it inconvenient",
      },
      {
        color: "blue",
        text: "Pro features are prominently displayed in the UI",
      },
      {
        color: "yellow",
        text: "Very poor scrolling performance with medium to large libraries",
      },
      {
        color: "red",
        text: "Signing out does not work",
      },
      {
        color: "blue",
        text: "Downloads have not been tested",
      },
    ],
    features: {
      tested: {
        wasTested: true,
        date: "2025-09-07",
        usedApiCorrectly: 2,
        comments: [
          "Loads big chunks of books in advance. Loads whole books in advance",
          "Continuously fetches whole books without doing anything around every 5 seconds. This data can be up to MB in size",
          "Uses old playback API",
          "Uses outdated authentication method",
        ],
      },
      ebooks: {
        available: false,
        ePubSupport: false,
        pdfSupport: false,
        annotations: false,
      },
      audiobooks: true,
      podcasts: false,
      queue: false,
      autoqueue: false,
      sleepTimer: true,
      chapters: true,
      playHistory: false,
      shakeToRewind: false,
      downloads: true,
      customHeaders: false,
      caching: true,
      biggerScreens: true,
      oidc: {
        available: false,
      },
      carSupport: undefined,
      widgets: false,
      bookmarks: false,
    },
  },
  {
    name: "Prologue",
    link: "https://prologue.audio/",
    iconLink: undefined,
    language: "Unknown",
    OSes: ["iOS"],
    openSource: false,
    cost: "Freemium",
    notes: [
      {
        color: "green",
        text: "Has advanced features like Volume Boost, EQ, and Media Info",
      },
      {
        color: "green",
        text: "Supports watchOS",
      },
      {
        color: "green",
        text: "Current Testflight contains no paid features",
      },
      {
        color: "red",
        text: "Loads most server information directly into the app. An app should never need to have that much information pre-loaded. Especially since the app uses very little pagination causing much traffic. I never got the app load all data of my only 1.5 year old account",
      },
    ],
    features: {
      tested: {
        wasTested: true,
        date: "2025-09-28",
        usedApiCorrectly: 1,
        comments: [
          "Loads the whole library in advance",
          "Uses old playback API",
          "Does load all sessions of a user in advance (only in 10 session chunks, for the whole user containing thousands of sessions) - Bad in general",
          "Does not use the /play endpoint, but uses sessions. This can lead to overwriting progress made on other clients",
        ],
      },
      ebooks: {
        available: false,
        ePubSupport: false,
        pdfSupport: false,
        annotations: false,
      },
      audiobooks: true,
      podcasts: false,
      queue: false,
      autoqueue: false,
      sleepTimer: true,
      chapters: true,
      playHistory: true,
      shakeToRewind: undefined,
      downloads: true,
      customHeaders: true,
      caching: true,
      biggerScreens: true,
      oidc: {
        available: true,
        mobileRedirectURI: "prologue://oauth",
      },
      carSupport: true,
      widgets: true,
      bookmarks: true,
    },
  },
  {
    name: "AudioBooth",
    link: "https://github.com/AudioBooth/AudioBooth",
    iconLink: "./logos/AudioBooth.png",
    language: "Swift",
    OSes: ["iOS"],
    openSource: true,
    cost: "Free",
    notes: [
      {
        color: "green",
        text: "Is the only free third party iOS client for Audiobookshelf.",
      },
    ],
    features: {
      tested: {
        wasTested: true,
        date: "2026-01-04",
        usedApiCorrectly: 5,
      },
      ebooks: {
        available: true,
        ePubSupport: true,
        pdfSupport: true,
        annotations: false,
      },
      audiobooks: true,
      podcasts: false,
      queue: false,
      autoqueue: false,
      sleepTimer: true,
      chapters: true,
      playHistory: true,
      shakeToRewind: false,
      downloads: true,
      customHeaders: true,
      caching: true,
      biggerScreens: true,
      oidc: {
        available: true,
        mobileRedirectURI: "audiobooth://oauth",
      },
      carSupport: true,
      widgets: true,
      bookmarks: true,
    },
  },
  {
    name: "Watch Only! - Auribook",
    link: "https://auribook.backlog.workers.dev/",
    iconLink: undefined,
    language: "Unknown",
    OSes: ["iOS"],
    openSource: false,
    cost: "Paid",
    notes: [
      {
        color: "blue",
        text: "I do not have an Apple Watch and cannot test the app. The author of the app did not respond to my message asking which endpoints are used. All features are taken from the website",
      },
      {
        color: "yellow",
        text: "There is currently no public way to easily report or view open issues other than privately by email",
      },
    ],
    features: {
      tested: {
        wasTested: false,
      },
      ebooks: {
        available: false,
      },
      audiobooks: true,
      podcasts: undefined,
      queue: undefined,
      autoqueue: undefined,
      sleepTimer: true,
      chapters: undefined,
      playHistory: undefined,
      shakeToRewind: undefined,
      downloads: undefined,
      customHeaders: undefined,
      caching: undefined,
      biggerScreens: true,
      oidc: undefined,
      carSupport: undefined,
      widgets: undefined,
      bookmarks: undefined,
    },
  },
  {
    name: "Toutui",
    link: "https://github.com/AlbanDAVID/Toutui",
    iconLink: undefined,
    language: "Rust",
    OSes: ["Linux", "macOS"],
    openSource: true,
    cost: "Free",
    notes: [
      {
        color: "red",
        text: "Loads the whole library without consulting the server afterwards. Imo an app never should have the whole state of the server/library loaded",
      },
    ],
    features: {
      tested: {
        wasTested: true,
        date: "2025-10-14",
        usedApiCorrectly: 3,
        comments: [
          "2.5",
          "Loads the whole library in advance",
          "Uses old playback API",
          "Does use /sync and /progress endpoint together. This can lead to serious issues with double progress due to race conditions. Newer server versions fix this automatically every restart",
        ],
      },
      ebooks: {
        available: false,
      },
      audiobooks: true,
      podcasts: true,
      queue: false,
      autoqueue: false,
      sleepTimer: false,
      chapters: true,
      playHistory: false,
      shakeToRewind: false,
      downloads: false,
      customHeaders: false,
      caching: false,
      biggerScreens: true,
      oidc: {
        available: false,
      },
      carSupport: false,
      widgets: false,
      bookmarks: false,
    },
  },
  {
    name: "Still",
    link: "https://testflight.apple.com/join/JgVXybzQ",
    iconLink: undefined,
    language: "Unknown",
    OSes: ["iOS"],
    openSource: false,
    cost: "Free",
    notes: [
      {
        color: "green",
        text: "See first comment. Does one of the best session managements I have seen so far",
      },
      {
        color: "blue",
        text: "See first comment. App is in early TestFlight. So any features tested are a first iteration and probably will improve quickly",
      },
    ],
    features: {
      tested: {
        wasTested: true,
        date: "2025-10-19",
        usedApiCorrectly: 4,
        comments: [
          "Uses old playback API",
          "Fetches every book as expanded independent and it's progress in two separate requests (Uses caching, but will still lead to a lot of requests and data usage). No pagination in e.g. series view",
        ],
      },
      ebooks: {
        available: false,
      },
      audiobooks: true,
      podcasts: false,
      queue: false,
      autoqueue: false,
      sleepTimer: true,
      chapters: true,
      playHistory: false,
      shakeToRewind: false,
      downloads: true,
      customHeaders: false,
      caching: true,
      biggerScreens: true,
      oidc: {
        available: false,
      },
      carSupport: undefined,
      widgets: undefined,
      bookmarks: true,
    },
  },
  {
    name: "Watch Only! - ShelfTime",
    link: "https://github.com/mkaflowski/ShelfTime",
    iconLink: undefined,
    language: "Kotlin",
    OSes: ["Android"],
    openSource: true,
    cost: "Free",
    notes: [
      {
        color: "blue",
        text: "Bigger screen is true due to being a watch app",
      },
      {
        color: "red",
        text: "No stats will be generated (even if progress would sync) because sessions are not used at all",
      },
    ],
    features: {
      tested: {
        wasTested: true,
        date: "2025-10-20",
        usedApiCorrectly: 0,
        comments: [
          "Uses old authentication method",
          "Uses old playback API",
          "Does not use any sessions. Does use /progress endpoint (which should not be used for this), but still for me there was no progress synced",
          "Did only 'sync' (I saw the request, but as mentioned, no actual progress was updated) when switching books or chapters",
          "Does make requests without an itemId, which leads to 404 (could lead to bans for stricter setups)",
          "Loads all items at once without pagination",
          "Downloads did not work at all for me, but I see that a request is being made (but not the /download endpoint)",
        ],
      },
      ebooks: {
        available: false,
      },
      audiobooks: true,
      podcasts: false,
      queue: false,
      autoqueue: false,
      sleepTimer: false,
      chapters: true,
      playHistory: false,
      shakeToRewind: false,
      downloads: false,
      customHeaders: false,
      caching: true,
      biggerScreens: true,
      oidc: {
        available: false,
      },
      carSupport: false,
      widgets: false,
      bookmarks: false,
    },
  },
  {
    name: "SideShelf",
    link: "https://testflight.apple.com/join/X6V7p1sD",
    iconLink: undefined,
    language: "Unknown",
    OSes: ["iOS"],
    openSource: true,
    cost: "Freemium",
    heavyAiUsage: true,
    notes: [
      {
        color: "red",
        text: "App did not load at all for me. It correctly authenticated and displayed the amout of libraries, but failed to display any content.",
      },
      {
        color: "red",
        text: "I am not sure if E-books are supported, but I do not added to have unknown there. Because I cannot test the app at all I put in false at the moment.",
      },
    ],
    features: {
      tested: {
        wasTested: false,
      },
      ebooks: {
        available: false,
      },
      audiobooks: undefined,
      podcasts: undefined,
      queue: undefined,
      autoqueue: undefined,
      sleepTimer: undefined,
      chapters: undefined,
      playHistory: undefined,
      shakeToRewind: undefined,
      downloads: undefined,
      customHeaders: undefined,
      caching: undefined,
      biggerScreens: undefined,
      oidc: undefined,
      carSupport: undefined,
      widgets: undefined,
      bookmarks: undefined,
    },
  },
  {
    name: "Voca Player",
    link: "https://apps.velosec.au/voca/",
    iconLink: undefined,
    language: "Swift, Kotlin",
    OSes: ["iOS", "Android"],
    openSource: false,
    cost: "Freemium",
    heavyAiUsage: true,
    notes: [
      {
        color: "green",
        text: "Supports Apple CarPlay and Apple Watch with standalone offline playback",
      },
    ],
    features: {
      tested: {
        wasTested: false,
      },
      ebooks: {
        available: true,
        ePubSupport: true,
        pdfSupport: true,
        annotations: false,
      },
      audiobooks: true,
      podcasts: true,
      queue: false,
      autoqueue: false,
      sleepTimer: true,
      chapters: true,
      playHistory: true,
      shakeToRewind: false,
      downloads: true,
      customHeaders: false,
      caching: false,
      biggerScreens: true,
      oidc: {
        available: false,
      },
      carSupport: true,
      widgets: false,
      bookmarks: true,
    },
  },
];
