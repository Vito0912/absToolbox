...
export async function executePathTagUpdater(
  ...
  for (const ruleSet of ruleSets) {
    ...
    for (const book of books) {
      ...
      console.log(
        `Checking book ${book.media.metadata.title} with path ${book.path} against regex ${regex}`,
      );
        ...
        type == "tags"
          ...
          : payload.push({
              ...
              mediaPayload: {
                metadata: {
                  genres: [
                    ...new Set([...(book.media?.metadata?.genres || []), tag]),
                  ],
                },
              },
            });
    ...
    if (payload.length > 0 && !dryRun) {
      try {
        ...
        throw new Error(
          `Failed to update books for rule "${ruleSet}": ${error instanceof Error ? error.message : String(error)}`,
        );
      }
    }
    if (dryRun) {
