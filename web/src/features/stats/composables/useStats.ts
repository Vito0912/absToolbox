...
import { AbsUser } from "@vito0912/abs-ts-sdk";
import {
  AbsLibraryItemExpanded,
  AbsListeningSession,
  AbsListeningSessionBookMediaMetadata,
  AbsListeningSessionPodcastMediaMetadata,
...
export function useStats() {
  ...
  const userData = ref<AbsUser | null>(null);
  const sessions = ref<AbsListeningSession[]>([]);
  const libraryItems = ref<Map<string, AbsLibraryItemExpanded>>(new Map());
  const config = ref<StatsConfig>({ ...defaultStatsConfig });
  const loadingProgress = ref({ current: 0, total: 0, status: "" });

  const loadUserData = async (): Promise<AbsUser> => {
    loadingProgress.value.status = "Loading user data...";
    const response = await absClient.users.me();
    return response;
  };

  const loadAllSessions = async (
    _userId: string,
  ): Promise<AbsListeningSession[]> => {
    loadingProgress.value.status = "Loading listening sessions...";
    const listeningSessions = await absClient.users.listeningSessions({
      itemsPerPage: 1000000,
    });
    return listeningSessions.sessions || [];
  };

  const loadLibraryItemsBatch = async (
    libraryItemIds: string[],
  ): Promise<Map<string, AbsLibraryItemExpanded>> => {
    const itemMap = new Map<string, AbsLibraryItemExpanded>();
    ...
    for (let i = 0; i < uniqueIds.length; i += batchSize) {
      ...
      try {
        const allItems = await absClient.libraryItems.batchGet({
          libraryItemIds: batch,
        });

        const items = allItems.libraryItems || [];
  ...
  const filterSessionsByDate = (
    allSessions: AbsListeningSession[],
    startDate: string,
    endDate: string,
  ): AbsListeningSession[] => {
  ...
  const processStats = async (
    ...
    user: AbsUser,
    cfg: StatsConfig,
    items: Map<string, AbsLibraryItemExpanded> = new Map(),
    ...
    for (const session of filteredSessions) {
      ...
      const bookSessionMetadata =
        session.mediaMetadata as AbsListeningSessionBookMediaMetadata;
      const podcastSessionMetadata =
        session.mediaMetadata as AbsListeningSessionPodcastMediaMetadata;

      const genres =
        bookMetadata?.genres ||
        bookSessionMetadata?.genres ||
        podcastSessionMetadata?.genres ||
        [];
      ...
      const authors =
        bookMetadata?.authors || bookSessionMetadata?.authors || [];
      ...
      const narrators =
        bookMetadata?.narrators || bookSessionMetadata?.narrators || [];
      ...
      const series = bookMetadata?.series || bookSessionMetadata?.series || [];
